...
We write markdown documentation as we feel necessary. Document each directory using `README.md` files and 
if possible, link them together via `SUMMARY.md` in the root folder. There may be multiple `README-*.md` files which can be 
linked from each other. However, each of the files must be mentioned (linked to) in `SUMMARY.md`.
...
Although the documentation root is in `book`, the `serve.sh` script ensures the live reloading of the book even if the files
outside of the `book` directory change. To do so, it uses `inoticoming` file watcher for relevant files and notifies gitbook
to update, if a file has changed. Live reloading is an optional feature which does not necessarily work correctly in 
all cases (graphs/UMLs update). In case it did not run correctly, kill the `serve.sh` script and run it again.
