...
Running tests requires a set up database. The default settings that are used are database location `localhost:5433` and password `breviary-db-pass`. The defaults can be changed by setting the environment variables `DB_URL` and `DB_PASSWORD`.
